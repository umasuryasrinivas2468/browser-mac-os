...
import { X, Search, Bot, Loader2, AlertCircle, FileText, Presentation, Download } from 'lucide-react';
...
const SearchPopup: React.FC<SearchPopupProps> = ({ isOpen, onClose, initialQuery = '' }) => {
  ...
  const generatePDF = (content: string, title: string = 'Search Results') => {
    try {
      const pdf = new jsPDF();
      ...
      const lines = pdf.splitTextToSize(content.replace(/\*\*(.*?)\*\*/g, '$1'), maxWidth);
  ...
  // Generate PowerPoint-style HTML for presentation
  const generatePPT = (content: string, title: string = 'Search Results') => {
    try {
      // Split content into slides based on paragraphs or sections
      ...
      const htmlContent = `
        ...
        <head>
          ...
          <style>
            ...
            .slide h1 { color: #0066cc; font-size: 3em; margin-bottom: 30px; }
            .slide h2 { color: #0066cc; font-size: 2em; margin-bottom: 20px; }
            .slide p { font-size: 1.5em; line-height: 1.6; margin-bottom: 20px; }
            .slide:first-child { background: linear-gradient(135deg, #0066cc, #004499); color: white; text-align: center; }
            .slide:first-child h1 { color: white; font-size: 4em; }
        ...
        <body>
          <div class="slide">
            <h1>${title}</h1>
            <p>Generated by Aczen OS AI Search</p>
          </div>
          ${slides.map(slide => `
            <div class="slide">
              ${slide.includes('**') ? 
                `<h2>${slide.replace(/\*\*(.*?)\*\*/g, '$1').split('\n')[0]}</h2>
                 <p>${slide.replace(/\*\*(.*?)\*\*/g, '').split('\n').slice(1).join('<br>')}</p>` :
                `<p>${slide.replace(/\n/g, '<br>')}</p>`
              }
            </div>
          `).join('')}
  ...
  const handleGeneratePDF = () => {
    if (result) {
      const success = generatePDF(result, query || 'Search Results');
      if (success) {
  ...
  const handleGeneratePPT = () => {
    if (result) {
      const success = generatePPT(result, query || 'Search Results');
      if (success) {
